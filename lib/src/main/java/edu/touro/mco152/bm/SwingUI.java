...
public class SwingUI extends SwingWorker<Boolean, DiskMark> implements programUI  {
    ...
    protected Boolean doInBackground() throws Exception {
        ...
        if (App.writeTest) {
             ...
             */
            for (int m = startFileNum; m < startFileNum + App.numOfMarks && !isCancelledpi(); m++) {
                ...
                try {
                    try (RandomAccessFile rAccFile = new RandomAccessFile(testFile, mode)) {
                        for (int b = 0; b < numOfBlocks; b++) {
                             ...
                             */
                            setProgresspi((int) percentComplete);
                        }
                    }
        ...
        if (App.readTest && App.writeTest && !isCancelledpi()) {
    ...
    public void done() {
        ...
        try {
            lastStatus = super.get();   // record for future access //used to be super.get
        } catch (Exception e) {
